...
export async function GET(request: NextRequest) {
  try {
    ...
    // Fix 5: Replace the second groupBy (which loaded every unique recipient
    // into memory just to call .length on the result) with a single
    // COUNT DISTINCT SQL query. ~40% less DB work per audience page load.
    const domainIds = domains.map((d) => d.id)

    let totalUniqueRecipients: number
    if (isAdmin && domains.length > 1) {
      const [row] = await prisma.$queryRawUnsafe<[{ total: bigint }]>(
        search
          ? `SELECT COUNT(DISTINCT "to")::bigint AS total
             FROM "Email"
             WHERE "domainId" = ANY($1)
               AND "to" IS NOT NULL
               AND "to" ILIKE $2`
          : `SELECT COUNT(DISTINCT "to")::bigint AS total
             FROM "Email"
      ...
      totalUniqueRecipients = Number(row.total)
    } else {
      const [row] = await prisma.$queryRawUnsafe<[{ total: bigint }]>(
        search
          ? `SELECT COUNT(DISTINCT "to")::bigint AS total
    ...
    // domainIds already declared above in the COUNT DISTINCT block (Fix 5)
    ...
    const overviewStats = {
      // Fix 5: totalUniqueRecipients is now a number, not an array
      totalRecipients: totalUniqueRecipients,
    ...
    return cachedResponse(
      ...
      pagination: {
        // Fix 5: totalUniqueRecipients is now a number, not an array
        total: totalUniqueRecipients,
        page: page,
        limit: limit,
        totalPages: Math.ceil(totalUniqueRecipients / limit),
        hasMore: totalUniqueRecipients > offset + limit,
        hasPrevious: page > 1,
      },
